...
#define NUMBER_OF_PASSENGER_SEATS 5
#define NUMBER_OF_MUTEXES 5
#define SLEEP_ON_ITERATION_RENDER 20000
#define PLANE_INIT_COORD_X 480
#define PLANE_INIT_COORD_Y 610
...
int passengers_count_in_cities[3];

pthread_mutex_t plane_mutexes[5];
...
void passengers_thread_routine(void *data) {
    ...
    while (true) {
        ...
        if (
            ...
            //Перебор всех 5 сидений в самолёте на наличие свободных.
            for (int i = 0; i < 5; i++) {
...
void render_cities() {
    ...
    values.foreground = 0xffffff;
    XChangeGC(display, gc, GCForeground, &values);
    XDrawString(display, window, gc, 855, 115, "London", 6);
    

    values.foreground = 0x000000;
    XChangeGC(display, gc, GCForeground, &values);
    XFillArc(display, window, gc, 445, 550, 140, 140, 360 * 64, 360 * 64);
    values.foreground = 0xffffff;
    XChangeGC(display, gc, GCForeground, &values);
    XDrawString(display, window, gc, 500, 610, "Moscow", 6);
//------------------------------------------------------------------------------------
    values.foreground = 0x000000;
    XChangeGC(display, gc, GCForeground, &values);
    XDrawLine(display, window, gc, 120, 120, 840, 120);//---------new-line-top-------
    XDrawLine(display, window, gc, 515, 610, 840, 120);//---------new-line-right------
    XDrawLine(display, window, gc, 515, 610, 120, 80);//---------new-line-left-------
...
void render_information_table() {
    ...
    XDrawLine(display, window, gc, 370, 295, 620, 295);

    XDrawLine(display, window, gc, 620, 220, 620, 295);
    XDrawLine(display, window, gc, 520, 220, 520, 295);
    XDrawLine(display, window, gc, 420, 220, 420, 295);
    XDrawLine(display, window, gc, 370, 220, 370, 295);
...
void render_passengers_count_in_cities() {
    ...
    int digits_count_of_london_passengers_number = passengers_count_in_cities[1] > 9 ? 2 : 1;
    ...
    sprintf(london_passengers_count_buffer, "%d", passengers_count_in_cities[1]);
    ...
    london_message_coord.x = 610;
    london_message_coord.y = 65;
